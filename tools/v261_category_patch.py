...
new = '''<select id="cat" multiple size="4" title="Giữ Ctrl để chọn nhiều danh mục"><option value="0">-- Không chọn danh mục --</option><?php foreach($cats as $cat)echo '<option value="'.esc_attr($cat->term_id).'">'.esc_html($cat->name).'</option>';?></select> <span style="font-size:12px;color:#666">Giữ Ctrl để chọn nhiều danh mục</span> <label><input id="ai" type="checkbox" checked> GPT viết lại SEO</label>'''
if old not in s:
...
s = s.replace('id="cat" multiple size="4"', 'id="cat" class="xdn-cat-select" multiple size="4"', 1)
